...
const CommentListItem = ({ comment, depth, onLoadMoreReplies }: CommentListItemProps) => {
  ...
  const repliesToShow = showReplies ? comment.replies : comment.replies.slice(0, INLINE_REPLIES_LIMIT);
...
const goToRepliesPage = () => {
  ...
  return (
    <View style={[styles.commentContainer, { marginLeft: depth * 15 }]}>
      ...
      {canShowRepliesInline && comment.replies.length > 0 && (
        <>
          ...
          {hasMoreInlineReplies && !showReplies && (
            <Pressable onPress={() => setShowReplies(true)} style={styles.showReplies}>
